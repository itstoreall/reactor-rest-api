...
- node-cache npm - пакет для работы с кэшем

Сущность проекта (MongoDB)
...
db.<collection>.insert({name: 'Barsik', age: 4}) (добавление данных в коллекцию)
db.<collection>.updateOne(...); (v 3.2 >) (замена одного)
db.<collection>.updateMany(...); (v 3.2 >) (множественная замена)
db.<collection>.remove({age: 5}); (удаление данных)

Mongoose ODM:
.find(), findOne() (вторым параметром передаются поля котрорые нужно получить из документа)
.exec() - выполнить (замена промису)
.create() (запись в базу. Вызывается на модели)
.save() (второй способ записи в базу)
.update() (обновляет запись. Вызывается на модели)
.remove() (удалить запись. Вызывается на модели)
